...
void printScreen() {
    ...
    u8g2.clearBuffer();
    //old if statement
    //(machineState == kPidNormal || (machineState == kBrew && FEATURE_SHOTTIMER == 0) || ((machineState == kPidDisabled) && FEATURE_PIDOFF_LOGO == 0)) && (machineState == kManualFlush) ||
    ...
    if (machineState == kWaterTankEmpty) {// Water empty
        ...
        else if (FEATURE_HEATINGLOGO > 0 && machineState == kPidNormal && (setpoint - temperature) > 0.3) {
        // For status info
